...
func main() {
	...
	alnur := User{Name: "Alnur", Email: "alnur@example.com", Balance: 1000}
	dias := User{Name: "Dias", Email: "dias@example.com", Balance: 500}

	if err := InsertUser(db, alnur); err != nil {
		log.Fatalf("InsertUser(Alnur): %v", err)
	}
	if err := InsertUser(db, dias); err != nil {
		log.Fatalf("InsertUser(Dias): %v", err)
	}
